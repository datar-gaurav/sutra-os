...
export default function SettingsPage() {
    ...
    const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>({ Integrations: true });
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8 animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <select value={newProviderType} onChange={(e) => {
                                    ...
                                    setNewSupportsTools(e.target.value !== "perplexity" && e.target.value !== "openai_compatible");
            ...
            {/* Configuration (formerly Environment Variables) */}
            <div className="glass-card p-6">
                ...
                <div className="space-y-4">
                    {sortedEnvGroups.map(group => {
                        ...
                        const items = envByGroup[group];

                        // For integrations, further sub-group by service
                        const subGroups: Record<string, EnvVarItem[]> = {};
                        if (group === "Integrations") {
                            for (const item of items) {
                                let subName = "Other";
                                for (const [prefix, name] of Object.entries(INTEGRATION_SUBGROUPS)) {
                        ...
                        return (
                            <div key={group} className="border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden">
                                <button
                                    ...
                                    className="w-full flex items-center justify-between p-3 bg-gray-50/50 dark:bg-surface-dark1 hover:bg-gray-100 dark:hover:bg-surface-dark2 transition-colors"
                                >
                                    ...
                                    <div className="p-4 space-y-6">
                                        {group === "Integrations" ? (
                                            Object.entries(subGroups).map(([subName, subItems]) => (
                                                <div key={subName}>
                                                    <h4 className="text-xs font-bold text-indigo-500/80 uppercase tracking-tight mb-3 flex items-center gap-2">
                                                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                                                        {subName}
                                                    </h4>
                                                    <div className="space-y-4 pl-3 border-l border-gray-100 dark:border-gray-800">
                                                        {subItems.map(envVar => renderEnvVar(envVar))}
                                                    </div>
                                                </div>
                                            ))
                                        ) : (
                                            <div className="space-y-4">
                                                {items.map(envVar => renderEnvVar(envVar))}
                                            </div>
                                        )}
                                    </div>
                                )}
            ...
            <div className="glass-card p-6">
                ...
                {showCreateKey && (
                    <form onSubmit={handleCreateApiKey} className="mb-4 p-4 rounded-xl border border-stone-300 dark:border-stone-700 bg-stone-100/50 dark:bg-stone-900/20 space-y-3">
                        ...
                        <div className="flex justify-end gap-2">
                            <button type="button" onClick={() => { setShowCreateKey(false); setCreatedKey(null); }} className="btn-secondary text-sm">Cancel</button>
    ...
    function renderEnvVar(envVar: EnvVarItem) {
        ...
        return (
            <div key={envVar.key} className="grid grid-cols-[1fr_auto] gap-3 items-start">
                {/* Label + description */}
