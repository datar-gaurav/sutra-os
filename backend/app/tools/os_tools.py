...
# Commands that agents are allowed to execute.  Only the base binary is
# checked — arguments are passed through but shell meta-characters are
# never interpreted because we always use shell=False.
ALLOWED_COMMANDS: set[str] = {
    "ls", "cat", "head", "tail", "wc", "grep", "find", "du", "df",
    "date", "whoami", "hostname", "uname", "env", "echo", "pwd",
    "git", "python", "python3", "pip", "node", "npm", "npx",
    "curl", "wget", "jq", "sort", "uniq", "cut", "tr", "sed", "awk",
...
def run_shell_command(command: str, working_directory: str = "~") -> str:
    ...
    Only commands in the server-side allowlist are permitted.
    Shell meta-characters (pipes, redirects, etc.) are NOT interpreted.
    ...
    try:
        ...
        if not _is_path_allowed(cwd):
            return f"Access Denied: working directory '{working_directory}' is outside allowed paths."
