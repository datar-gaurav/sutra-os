...
async def run_daily_analysis() -> EvolveRun:
    ...
    # Step 2: Gather agent-level error patterns
    try:
    ...
    # Step 2b: Gather platform-level system errors (routes, background tasks, startup)
    try:
        from app.tools.evolve_tools import _get_system_errors
        system_errors_json = await _get_system_errors(hours=24, limit=50)
        analysis_data["system_errors"] = json.loads(system_errors_json)
    except Exception as e:
        errors.append(f"system_errors: {e}")
        logger.warning(f"[Evolve] System errors failed: {e}")
