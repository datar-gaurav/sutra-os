...
class LLMRegistry:
    ...
    def get_chat_model(
        ...
        if provider == "ollama":
            ...
            api_key = self._get_api_key(provider, settings.nvidia_nim_api_key)
            return ChatOpenAI(
                model=model,
                api_key=api_key,
            ...
            # Generic OpenAI-compatible provider — reads base_url from registry
            api_key = self._get_api_key(provider)
            config = self._providers.get(provider, {})
            base_url = config.get("base_url", "") or "http://localhost:8080/v1"
            return ChatOpenAI(
                model=model,
                api_key=api_key,
                base_url=base_url,
    ...
    async def fetch_nvidia_nim_models(self) -> list[dict]:
        ...
        import os as _os
        api_key = (
            self._providers.get("nvidia_nim", {}).get("api_key", "")
            or _os.environ.get("NVIDIA_NIM_API_KEY", "")
            or settings.nvidia_nim_api_key
        )
        if not api_key:
            logger.warning("No NVIDIA NIM API key configured — cannot fetch model list.")
            return []
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(
            ...
            logger.warning(f"Failed to fetch NVIDIA NIM models: {e}")
            return []
