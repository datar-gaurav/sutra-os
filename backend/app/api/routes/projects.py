...
def _secure_filename(filename: str) -> str:
    ...
    # Normalise unicode, strip path separators, collapse whitespace
...
def _assert_path_contained(child: str, parent: str) -> None:
    """Raise 403 if *child* escapes *parent* after symlink resolution."""
...
async def upload_project_file(
    ...
    # Sanitise filename and enforce size limit
    safe_name = _secure_filename(file.filename or "unnamed")
    content = await file.read()
    if len(content) > _MAX_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="File too large (max 50 MB)")

    # Save file to disk — ensure path stays inside files_dir
    file_path = os.path.join(files_dir, safe_name)
    _assert_path_contained(file_path, files_dir)
    ...
    pf = ProjectFile(
        project_id=project_id,
        file_name=safe_name,
        file_path=safe_name,  # relative path (sanitised)
...
async def download_project_file(project_id: str, file_id: str, db: AsyncSession = Depends(get_db)):
    ...
    _assert_path_contained(full_path, project.files_dir)
