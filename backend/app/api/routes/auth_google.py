...
def _sign_state(raw: str) -> str:
    """Return 'raw.signature' where signature is an HMAC-SHA256 hex digest."""
    sig = hmac.new(settings.secret_key.encode(), raw.encode(), hashlib.sha256).hexdigest()[:16]
    return f"{raw}.{sig}"


def _verify_state(signed: str) -> str:
    """Verify and return the raw state, or raise HTTPException 400."""
    if "." not in signed:
        raise HTTPException(400, "Invalid OAuth state")
    raw, sig = signed.rsplit(".", 1)
    expected = hmac.new(settings.secret_key.encode(), raw.encode(), hashlib.sha256).hexdigest()[:16]
...
async def google_login(request: Request, agent_id: str | None = None, service: str = "gmail"):
    ...
    state = _sign_state(f"{agent_id or ''}:{service}:{frontend_origin}")
...
async def google_callback(
    ...
    raw_state = _verify_state(state) if state else ""
    # Format: "<agent_id>:<service>:<frontend_origin>"
    parts = raw_state.split(":", 2) if raw_state else []
    if len(parts) >= 3:
